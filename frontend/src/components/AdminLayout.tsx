...
import {
  Menu as MenuIcon, Dashboard, People, School, EventNote, BarChart,
  ArrowBack, Logout, AdminPanelSettings, DarkMode, LightMode, Notifications,
  Person,
...
const adminNav = [
  ...
  { text: 'Sessions', icon: <EventNote />, path: '/admin/sessions' },
  ...
  const mobileDrawer = (
    <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Box sx={{ px: 3, pb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
        <Typography variant="h5" sx={{ fontWeight: 800, background: `linear-gradient(135deg, ${theme.palette.primary.main}, ${theme.palette.secondary.main})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', cursor: 'pointer' }} onClick={() => { navigate('/admin'); setMobileOpen(false); }}>Mentr</Typography>
        <Chip label="ADMIN" size="small" color="error" sx={{ height: 20, fontSize: '0.65rem', fontWeight: 700 }} />
      </Box>
      <List sx={{ px: 1, flex: 1 }}>
        {adminNav.map((item) => (
          <ListItemButton key={item.text} onClick={() => { navigate(item.path); setMobileOpen(false); }} selected={location.pathname === item.path}
            sx={{ borderRadius: 2, mb: 0.5, px: 2, py: 1, minHeight: 48,
              '&.Mui-selected': { bgcolor: theme.palette.error.main + '15', color: theme.palette.error.main, '& .MuiListItemIcon-root': { color: theme.palette.error.main }, borderLeft: `3px solid ${theme.palette.error.main}` },
              '&:hover': { bgcolor: theme.palette.error.main + '10' }, transition: 'all 0.25s cubic-bezier(0.22, 1, 0.36, 1)' }}>
                ...
                return (
                  <Box key={item.text} sx={{ position: 'relative', px: 1.4, py: 0, display: 'flex', alignItems: 'center', height: NAV_HEIGHT, cursor: 'pointer' }}>
                    <Typography onClick={() => navigate(item.path)}
                      sx={{ fontSize: '0.82rem', fontWeight: isActive ? 600 : 400, color: isActive ? theme.palette.error.main : isDark ? 'rgba(255,255,255,0.82)' : 'rgba(0,0,0,0.82)', cursor: 'pointer', transition: 'color 0.3s ease', whiteSpace: 'nowrap', letterSpacing: '-0.01em', '&:hover': { color: theme.palette.error.main } }}>
                      {item.text}
                    </Typography>
                    <Box sx={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', height: '2px', width: isActive ? '60%' : '0%', bgcolor: theme.palette.error.main, borderRadius: '1px', transition: 'width 0.35s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.3s ease', opacity: isActive ? 1 : 0 }} />
                  </Box>
                );
              })}
            </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            <Tooltip title="Account" {...TOOLTIP_PROPS}>
              <IconButton onClick={(e) => setAnchorEl(e.currentTarget)} size="small">
                <Avatar sx={{ width: 28, height: 28, bgcolor: theme.palette.error.main, fontSize: 12, fontWeight: 700 }}>
            ...
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}
              ...
              <MenuItem onClick={() => { setAnchorEl(null); navigate('/admin'); }} sx={{ py: 1.2, fontSize: '0.85rem' }}><Dashboard sx={{ mr: 1.5, fontSize: 18 }} /> Dashboard</MenuItem>
              <Divider />
              <MenuItem onClick={() => { setAnchorEl(null); navigate('/'); }} sx={{ py: 1.2, fontSize: '0.85rem' }}><ArrowBack sx={{ mr: 1.5, fontSize: 18 }} /> Back to App</MenuItem>
