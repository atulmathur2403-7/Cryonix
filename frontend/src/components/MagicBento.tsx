...
export const ParticleCard = ({
  ...
  return (
    <div
      ref={cardRef}
      className={`mentor-particle-container ${className}`}
      style={{ ...style, position: 'relative', overflow: 'hidden', '--bento-card-glow': glowColor } as React.CSSProperties}
      onClick={onClick}
    >
      {children}
    </div>
...
export const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const section = gridRef.current.closest('.mentor-bento-section');
      ...
      const cards = gridRef.current.querySelectorAll<HTMLElement>('.mentor-bento-card');
    ...
    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll<HTMLElement>('.mentor-bento-card').forEach(card => {
